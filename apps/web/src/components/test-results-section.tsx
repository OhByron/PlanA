  ...
  return (
    <>
        ...
        <section className="mb-8">
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800">{t('testResults.retestRequired')}</p>
                <p className="mt-1 text-xs text-amber-700">
                  {t('testResults.retestMessage', { count: testSummary!.fail + testSummary!.error })}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-3 text-xs text-gray-500">
                <span className="text-green-600">{testSummary.pass} {t('testResults.passed')}</span>
                {testSummary.fail > 0 && <span className="text-red-600">{testSummary.fail} {t('testResults.failed')}</span>}
                {testSummary.error > 0 && <span className="text-red-600">{testSummary.error} {t('testResults.errors')}</span>}
                {testSummary.skip > 0 && <span className="text-gray-400">{testSummary.skip} {t('testResults.skipped')}</span>}
              </div>
              {testSummary.lastRun && (
                <span className="ml-auto text-xs text-gray-400">
                  {t('testResults.lastRun', { date: new Date(testSummary.lastRun).toLocaleString() })}
                </span>
              )}
            </div>
          </div>
      ...
      {sourceTestResult && (
        <section className="mb-8">
          <h2 className="mb-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">
            {t('testResults.sourceTestFailure')}
          </h2>
          <div className="rounded-lg border border-red-200 bg-red-50 p-4">
            ...
            {sourceTestResult.suiteName && (
              <p className="mb-1 text-xs text-gray-500">{t('testResults.suite', { name: sourceTestResult.suiteName })}</p>
            )}
            {sourceTestResult.durationMs != null && (
              <p className="mb-2 text-xs text-gray-500">{t('testResults.duration', { ms: sourceTestResult.durationMs })}</p>
            )}
            ...
            <div className="mt-3 flex items-center justify-between">
              <p className="text-xs text-gray-400">
                {t('testResults.source', { source: sourceTestResult.source, date: new Date(sourceTestResult.reportedAt).toLocaleString() })}
              </p>
              <Button
                variant="ghost"
                size="sm"
              ...
              >
                {aiDefectLoading ? t('common.generating') : t('testResults.generateDescAC')}
              </Button>
            </div>
          </div>
        </section>
