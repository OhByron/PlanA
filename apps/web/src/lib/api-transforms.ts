...
export function toWorkItem(w: any): WorkItem & { itemNumber?: number | null } {
  return {
    ...
    title: w.title,
    description: w.description,
    workflowStateId: w.workflow_state_id,
    isCancelled: w.is_cancelled ?? false,
    stateName: w.state_name ?? '',
    stateSlug: w.state_slug ?? '',
    stateColor: w.state_color ?? '#6B7280',
    statePosition: w.state_position ?? 0,
    stateIsTerminal: w.state_is_terminal ?? false,
    stateIsInitial: w.state_is_initial ?? false,
    priority: w.priority,
